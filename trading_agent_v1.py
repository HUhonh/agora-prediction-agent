...
def record_trade_on_chain(side, size, price, btc_price=0):
    ...
    try:
        ...
        w3.eth.wait_for_transaction_receipt(txh, timeout=30)
        tx_hash = txh.hex()
        _settlement_tracker["arc_tx_hashes"].append(tx_hash)
        print(f"\n{'='*60}")
        print(f"  [Arc] ✅ 铸造证明代币成功!")
        print(f"  [Arc] Agent #{AGENT_ID} | {side} | {size} @ {price}")
        print(f"  [Arc] TX: https://testnet.arcscan.app/tx/{tx_hash}")
        print(f"{'='*60}\n")
        sys.stdout.flush()
        return tx_hash
    except:
